...
function waitForServer(child){
    return new Promise((resolve,reject)=>{
        ...
        child.stdout.on('data',chunk=>{if(chunk.toString().includes('Wintermaul v')){clearTimeout(timeout);resolve();}});
...
function connect(baseUrl,session,{release=RELEASE.releaseVersion,protocol=RELEASE.multiplayer.protocolId}={}){
    return new Promise((resolve,reject)=>{
        ...
        else url.searchParams.set('version',release);
            ...
            waitFor(predicate,timeoutMs=4000){
                ...
                return new Promise((resolveWait,rejectWait)=>{
                    ...
                    waiter.timer=setTimeout(()=>{
                        ...
                        rejectWait(new Error(`Timed out waiting for server message. Received: ${messages.map(message=>message.type).join(', ')||'nothing'}.`));
                    },timeoutMs);
                });
            },
...
(async()=>{
    ...
    try{
        ...
        const olderLegacyWelcome=await olderLegacy.waitFor(message=>message.type==='welcome');assert.equal(olderLegacyWelcome.protocolId,'wintermaul-mp/1');await olderLegacy.close();
        const wrongProtocol=await connect(baseUrl,'phase5-wrong-protocol-0001',{release:'0.98.0',protocol:'wintermaul-mp/2'});
        const wrongProtocolMessage=await wrongProtocol.waitFor(message=>message.type==='incompatible_version');assert.equal(wrongProtocolMessage.requiredProtocolId,'wintermaul-mp/1');await wrongProtocol.close();
